...
function InstallationDetails() {
  ...
  const acceptClicked = async() =>{
    try {
        ...
        if (response.status === 200) {
          ...
          swal("Success", "Request has been accepted successfully!", "success");
          navigate('/view-installation-requests');
        } else {
          swal("Error", "Failed to update cofirmation status", "error");
  ...
  const rejectClicked = async() =>{
    try {
      const response = await axios.patch(`http://localhost:8181/installation/${id}/decline`);
      
      if (response.status === 200) {
          console.log('Request declined and installation information deleted successfully');
          swal("Success", "Request has been rejected and mail has been sent to customer", "success");
          navigate('/view-installation-requests');
      } else {
        swal("Error", "Failed to decline installation", "error");

          console.log('Failed to decline installation and delete information');
          // Handle error or show an error message to the user
      }
  } catch (error) {
      console.error('Error declining installation:', error);
      swal("Error", "Failed to decline installation", "error");
    }
  }
  return (
