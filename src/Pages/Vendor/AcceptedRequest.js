  ...
  const completedClicked = async() =>{
    try {
        ...
        if (response.status === 200) {
          ...
          swal("Success", "Completion Status has been changed", "success");
          navigate('/installation-accepted-request'); 
        } else {
          swal("Error", "Failed to update confirmation status", "error");

          console.log('Failed to update confirmation status');
        }
      } catch (error) {
        swal("Error", "Error updating confirmation status", "error");
